...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Aggiunta della logica per salvare la risposta
    if (isset($data['contenuto']) && isset($data['codice_gioco'])) {
        $codiceGioco = $data['codice_gioco'];
        $contenutoRisposta = $data['contenuto'];
        $autoreRisposta = $_SESSION['username']; // Assumiamo che l'utente sia loggato
        ...
        // Troviamo la domanda corrispondente
        $domande = $dom->getElementsByTagName('domanda');
        foreach ($domande as $domanda) {
            if ($domanda->getElementsByTagName('codice_gioco')->item(0)->nodeValue == $codiceGioco) {
                // Creiamo un nuovo nodo risposta
                $risposta = $dom->createElement('risposta');
                $risposta->setAttribute('id', uniqid()); // Generiamo un ID unico per la risposta
                $risposta->appendChild($dom->createElement('contenuto', htmlspecialchars($contenutoRisposta)));
                $risposta->appendChild($dom->createElement('autore', htmlspecialchars($autoreRisposta)));
                $risposta->appendChild($dom->createElement('data', date('Y-m-d'))); // Aggiungiamo la data
...
<body>
                            ...
                            <div class="discussione">
                                <div class="discussione-header" style="display: flex; justify-content: space-between; align-items: center;">
                                    ...
                                    <div>
                                        ...
                                        <button class="btn-rispondi" onclick="apriFinestraRisposta('<?php echo htmlspecialchars($discussione['codice_gioco']); ?>', '<?php echo htmlspecialchars($discussione['autore']); ?>', '<?php echo htmlspecialchars($discussione['contenuto']); ?>')">Rispondi</button>
                                    </div>
                                </div>
                                        ...
                                        <?php if (!empty($discussione['risposte'])){ ?>
                                            <div class="risposte-container">
                                                ...
                                                <?php }
                                                    ...
                                                    if ($hiddenCount > 2){ // se ci sono più di 2 risposte, mostriamo pulsante ?>
                                            <button class="btn-mostra-altro" onclick="mostraAltreRisposte(this)">Mostra altre risposte</button>
                                            <div class="risposte-nascoste" style="display: none;">
                                                <?php 
                                                foreach ($discussione['risposte'] as $risposta){
    ...
    <script>
        ...
        function apriFinestraRisposta(codiceGioco, autore, contenutoDomanda) {
            document.getElementById('autoreRisposta').innerText = autore; 
            window.codiceGiocoRisposta = codiceGioco; 
            window.contenutoDomanda = contenutoDomanda;
        ...
        function inviaRisposta() {
            ...
            const codiceGioco = window.codiceGiocoRisposta; 
            const autore = '<?php echo $_SESSION['username']; ?>'; 
            const contenutoDomanda = window.contenutoDomanda;

            if (!contenuto) {
                alert("Il campo della risposta non può essere vuoto.");
                return;
            }
            ...
            const rispostaData = {
                ...
                contenuto: contenuto,
                autore: autore,
                contenuto_domanda: contenutoDomanda
            };

            fetch('invia_risposta.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
